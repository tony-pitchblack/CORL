...
class Actor(nn.Module):
    ...
    def mean_action(self, state: torch.Tensor) -> torch.Tensor:
        ...
        action = policy.mean
        return action.clamp(self._min_action, self._max_action)
...
class CriticRegularizedRegression:
    def __init__(
        self,
        actor: nn.Module,
    ...
    def _critic_loss(
        ...
        with torch.no_grad():
            next_actions = self._actor.mean_action(next_states)
            q_next_1 = self._target_critic_1(next_states, next_actions)
            q_next_2 = self._target_critic_2(next_states, next_actions)
    ...
    def _actor_loss(
        ...
        with torch.no_grad():
            pi_actions = self._actor.mean_action(states)
            q_pi_1 = self._critic_1(states, pi_actions)
            q_pi_2 = self._critic_2(states, pi_actions)
            v = torch.min(q_pi_1, q_pi_2)
            q_b_1 = self._critic_1(states, actions)
            q_b_2 = self._critic_2(states, actions)
        ...
        pi_pred = self._actor.mean_action(states)
...
def train(config: TrainConfig):
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        critic_1 = Critic(
            state_dim=state_dim, action_dim=action_dim, hidden_dim=config.hidden_dim
        )
        critic_2 = Critic(
            state_dim=state_dim, action_dim=action_dim, hidden_dim=config.hidden_dim
        )
        ...
        critic_1_optimizer = torch.optim.Adam(
            critic_1.parameters(), lr=config.critic_learning_rate
        )
        critic_2_optimizer = torch.optim.Adam(
            critic_2.parameters(), lr=config.critic_learning_rate
        )
        crr = CriticRegularizedRegression(
            actor=actor,
        ...
        for step in trange(config.num_train_steps, ncols=80):
            ...
            update_result = crr.update(batch)
            ...
            if (step + 1) % config.eval_frequency == 0:
                ...
                normalized_eval_scores = (
                    get_normalized_score(dataset, eval_scores) * 100.0
                )
                mlflow.log_metric(
                    "normalized_score",
