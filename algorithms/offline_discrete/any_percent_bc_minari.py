...
@dataclass
class TrainConfig:
    dataset_id: str = "minigrid/BabyAI-GoToObjS4/optimal-fullobs-v0"
    download: bool = True
    device: str = "cuda:1"
    seed: int = 0
    ...
    n_episodes: int = 10
    experiment_name: str = "BC-discrete-minari"
    run_name: str = "BC-discrete"
...
class DiscreteActor(nn.Module):
    ...
    @torch.no_grad()
    def act(self, state: np.ndarray, device: str = "cuda") -> int:
        state_t = torch.tensor(state.reshape(1, -1), device=device, dtype=torch.float32)
        logits = self(state_t)
        return int(torch.argmax(logits, dim=-1).cpu().numpy().item())
...
def train(config: TrainConfig):
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        if not isinstance(dataset.action_space, gym.spaces.Discrete):
            raise ValueError("This script supports only discrete action spaces.")
        ...
        n_actions = dataset.action_space.n
        ...
        actor = DiscreteActor(state_dim, n_actions).to(config.device)
